const ACCESSORY_PATTERN = /(A\/S|세척\s*(키트|액|제)|펜\s*솝|파우치|마스킹\s*테이프|루즈\s*시트|와시\s*노트|종이\s*컬렉션|서랍장|클립|잉크\s*세트|잉크세트|만년필용|만년필\s*제작\s*샘플)/i;
const FOUNTAIN_PEN_PATTERN = /만년필|(?<![A-Za-z])FP(?![A-Za-z])|fountain\s*pen/i;

const NAMED_NIBS = [
  ['NAGINATA_CONCORD', /나기나타\s*콘코드|naginata\s*concord/i],
  ['CROSS_CONCORD', /크로스\s*콘코드|cross\s*concord/i],
  ['CROSS_POINT', /크로스\s*포인트|cross\s*point/i],
  ['NAGINATA_CALLI', /나기나타\s*(캘리|calli)/i],
...
function relevantTail(name) {
  const marker = name.search(FOUNTAIN_PEN_PATTERN);
  const tail = marker >= 0 ? name.slice(marker) : name;
  return tail
    .replace(/\[[^\]]*\]/g, ' ')
    .replace(/\{[^}]*\}/g, ' ')
    .replace(/\s+/g, ' ')
...
export function extractNibSizes(productName) {
  ...
  if (!name || !FOUNTAIN_PEN_PATTERN.test(name) || ACCESSORY_PATTERN.test(name)) return [];

  const tail = relevantTail(name);
  const result = [];

  NAMED_NIBS.forEach(([id, pattern]) => {
    if (pattern.test(tail)) add(result, id);
  });

  const stubWord = /\b(?:stub|italic|이탤릭)\b/i.test(tail);
  if (stubWord) add(result, 'STUB');

  for (const match of tail.matchAll(/(?:^|[\s(/,+-])(0\.6|0\.85|0\.9|1\.0|1\.1|1\.3|1\.35|1\.4|1\.5|1\.6|1\.9|2\.0|2\.2|2\.3|2\.5|2\.7|2\.8|3\.2)(?:\s*mm)?(?=$|[\s)\],/+~-])/gi)) {
...
export function classifyNibCatalogRows(rows) {
  ...
  return { fountainPens, classified, unresolved };
}
