...
function renderMap(config){
  ...
  host.innerHTML=`
    <div class="live-map-head">
      ...
      <a href="../admin.html#store-map-admin" class="live-map-admin-link">안내도 관리</a>
    </div>
...
function insertMap(){
  ...
  if(!firstPanel||document.querySelector('#storeMap1FLive'))return;
  const section=document.createElement('section');
  section.className='tour-card live-map-card';
  section.id='storeMap1FLive';
  const intro=firstPanel.querySelector('.floor-intro-card');
