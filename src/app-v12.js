...
import { upgradePenModelFromGlb } from './pen-model-glb-v60.js';
...
const legacyKeys={cap_end:'cap_top',nib_grip:'grip_section',metal_parts:'center_ring_or_connector'};
const mobileMedia=matchMedia('(max-width:699px)');
const reducedMotion=matchMedia('(prefers-reduced-motion:reduce)');
const $=selector=>document.querySelector(selector);

const state={activePartId:parts[0].id,selections:{...defaultSelection},viewMode:'assembled',fullscreen:false,root:null,groups:null,partMeshes:new Map(),ready:false,renderFailed:false};
const initialQuery=new URLSearchParams(location.search);
for(const part of parts){const value=initialQuery.get(part.id)??initialQuery.get(part.canonicalId)??initialQuery.get(legacyKeys[part.id]);if(colors.some(color=>color.id===value&&color.group===part.colorGroup&&color.available!==false))state.selections[part.id]=value;}

const elements={viewerCard:$('#viewer-card'),stage:$('#viewer-stage'),canvas:$('#pen-canvas'),loading:$('#viewer-loading'),error:$('#viewer-error'),fallback:$('#static-pen-fallback'),partGrid:$('#part-grid'),palette:$('#palette'),summary:$('#summary-list'),staffSummary:$('#staff-summary'),staffDialog:$('#staff-dialog'),toast:$('#toast')};
...
function persist(){const query=new URLSearchParams(location.search);query.delete('c');query.delete('reset');query.set('lang',getLanguage());for(const part of parts){const value=state.selections[part.id];query.set(part.id,value);if(part.canonicalId&&part.canonicalId!==part.id)query.set(part.canonicalId,value);}history.replaceState(null,'',`${location.pathname}?${query}`);localStorage.setItem('blueblack-pen-combination-query',query.toString());dispatchEvent(new CustomEvent('combinationchange',{detail:{...state.selections}}));}
...
function renderStaticText(){
  ...
  $('#store-address').textContent=localizedAddress();$('#store-phone').textContent=STORE.phone;updateActiveBadge();
}

function updateActiveBadge(){if(!elements.stage)return;let badge=$('#viewer-active-part');if(!badge){badge=document.createElement('div');badge.id='viewer-active-part';badge.className='viewer-active-part';elements.stage.append(badge);}const labels={ko:'현재 선택 중',en:'SELECTING',ja:'選択中','zh-Hans':'当前选择','zh-Hant':'目前選擇'};badge.innerHTML=`<small>${labels[getLanguage()]??labels.ko}</small><strong>${localizePart(partById(state.activePartId)).name}</strong>`;}

function renderParts(){elements.partGrid?.replaceChildren(...parts.map((part,index)=>{const button=document.createElement('button');button.type='button';button.className='part-button';button.dataset.partId=part.id;button.dataset.canonicalPartId=part.canonicalId||part.id;button.setAttribute('role','tab');button.setAttribute('aria-selected',String(part.id===state.activePartId));button.innerHTML=`<span class="part-index">${index+1}</span><span>${localizePart(part).name}</span>`;button.addEventListener('click',()=>selectPart(part.id,true));return button;}));}

function swatch(color,part){const name=localizeColor(color);const selected=state.selections[part.id]===color.id;const button=document.createElement('button');button.type='button';button.className='swatch';button.dataset.colorId=color.id;button.setAttribute('role','radio');button.setAttribute('aria-checked',String(selected));button.setAttribute('aria-label',name);button.innerHTML=`${color.isNew?'<span class="new-tag">NEW</span>':''}<span class="swatch-check">✓</span><span class="swatch-dot" style="background:${color.hex}"></span><span>${name}</span>`;button.addEventListener('click',()=>{state.selections[part.id]=color.id;persist();applyColors();renderCustomizer();pulsePart();});return button;}
function paletteGroup(title,list,part){const section=document.createElement('section');section.className='palette-group';section.innerHTML=`<div class="palette-heading"><b>${title}</b><span>${list.length}</span></div>`;const grid=document.createElement('div');grid.className='swatch-grid';grid.setAttribute('role','radiogroup');grid.append(...list.map(color=>swatch(color,part)));section.append(grid);return section;}
function renderPalette(){if(!elements.palette)return;const part=partById(state.activePartId);const list=colors.filter(color=>color.group===part.colorGroup&&color.available!==false);if(part.colorGroup==='metal'){elements.palette.replaceChildren(paletteGroup(t('metalColors'),list,part));return;}elements.palette.replaceChildren(paletteGroup(t('originalColors'),list.filter(color=>!color.isNew),part),paletteGroup(t('newColors'),list.filter(color=>color.isNew),part));}

function fillSummary(target){if(!target)return;target.replaceChildren(...parts.map(part=>{const color=colorById(state.selections[part.id]);const item=document.createElement('div');item.className='summary-item';item.dataset.partId=part.id;item.dataset.canonicalPartId=part.canonicalId||part.id;item.innerHTML=`<span class="swatch-dot" style="background:${color.hex}"></span><div><small>${localizePart(part).name}</small><b>${localizeColor(color)}</b></div>`;item.tabIndex=0;item.setAttribute('role','button');item.addEventListener('click',()=>selectPart(part.id,true));item.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();selectPart(part.id,true);}});return item;}));}

function renderCustomizer(){const index=parts.findIndex(part=>part.id===state.activePartId);const part=parts[index];const localized=localizePart(part);const color=colorById(state.selections[part.id]);$('#part-name-en').textContent=part.nameEn.toUpperCase();$('#part-name').textContent=localized.name;$('#part-description').textContent=localized.description;$('#selected-dot').style.background=color.hex;$('#selected-color').textContent=localizeColor(color);$('#previous-part').disabled=index===0;$('#next-part').textContent=index===parts.length-1?t('finishSelection'):t('nextPart');renderStaticText();renderParts();renderPalette();fillSummary(elements.summary);fillSummary(elements.staffSummary);}
...
function configureResin(material,color,highlight=false){material.color.set(color.hex);material.metalness=0;material.roughness=color.roughness??.13;material.transparent=Boolean(color.transparent);material.opacity=color.transparent?.93:1;material.transmission=color.transparent?(color.transmission??.35):0;material.thickness=color.thickness??1.2;material.ior=1.48;material.depthWrite=true;material.emissive?.set(highlight?'#4b3819':'#000000');material.emissiveIntensity=highlight?.12:0;material.needsUpdate=true;}
function configureMetal(material,color,meshName,highlight=false){const dark=/slit|breather|feed|channel/i.test(meshName);material.color.set(dark?'#111419':color.hex);material.metalness=dark?.08:(color.metalness??.9);material.roughness=dark?.38:(color.roughness??.2);material.transparent=false;material.opacity=1;material.transmission=0;material.emissive?.set(highlight&&!dark?'#3e2e15':'#000000');material.emissiveIntensity=highlight&&!dark?.14:0;material.needsUpdate=true;}

function applyColors(){if(!state.ready)return;for(const part of parts){const selected=colorById(state.selections[part.id]);const active=part.id===state.activePartId;for(const mesh of state.partMeshes.get(part.id)??[]){const materials=Array.isArray(mesh.material)?mesh.material:[mesh.material];for(const material of materials){if(!material?.color)continue;if(part.colorGroup==='metal')configureMetal(material,selected,mesh.name,active);else configureResin(material,selected,active);}}}}
...
function fitWholePen(){
  ...
  resize();scene.updateMatrixWorld(true);
  const box=new THREE.Box3().setFromObject(state.root);if(box.isEmpty())return;
  const size=box.getSize(new THREE.Vector3());const center=box.getCenter(new THREE.Vector3());
  const verticalFov=THREE.MathUtils.degToRad(camera.fov);const horizontalFov=2*Math.atan(Math.tan(verticalFov/2)*Math.max(camera.aspect,.01));
  const fitHeight=size.y/(2*Math.tan(verticalFov/2));const fitWidth=size.x/(2*Math.tan(horizontalFov/2));
  const safetyMargin=state.fullscreen?1.16:(mobileMedia.matches?1.28:1.22);const distance=Math.max(fitHeight,fitWidth,size.z*2.4,120)*safetyMargin;
  camera.position.set(center.x,center.y+Math.min(8,distance*.035),center.z+distance);controls.target.copy(center);controls.minDistance=Math.max(70,distance*.48);controls.maxDistance=Math.max(360,distance*2.4);controls.update();
}
...
function selectPart(id,focus=false){if(!parts.some(part=>part.id===id))return;state.activePartId=id;applyColors();renderCustomizer();if(focus)focusPart(id,true);dispatchEvent(new CustomEvent('partchange',{detail:{partId:id,canonicalPartId:partById(id).canonicalId||id}}));}
...
function init3D(){
  ...
  try{
    ...
    const materials={customMaterial:resinMaterial,fixedMetal,darkInset,feedMaterial};
    const model=buildPenModel(materials);state.root=model.root;state.groups=model.groups;state.partMeshes=model.partMeshes;scene.add(state.root);state.ready=true;
    ...
    dispatchEvent(new Event('penviewerready'));dispatchEvent(new CustomEvent('viewerstatus',{detail:{status:'ready',source:'procedural'}}));
    upgradePenModelFromGlb({model}).then(upgraded=>{if(!upgraded)return;state.root=model.root;state.groups=model.groups;state.partMeshes=model.partMeshes;applyColors();applyViewMode();fitWholePen();renderFrame();dispatchEvent(new CustomEvent('viewerstatus',{detail:{status:'ready',source:'glb'}}));});
