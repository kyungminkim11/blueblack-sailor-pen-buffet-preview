...
for(const part of parts){const value=initialQuery.get(part.id)??initialQuery.get(legacyKeys[part.id]);if(colors.some(color=>color.id===value&&color.group===part.colorGroup))state.selections[part.id]=value;}
...
function persist(){const query=new URLSearchParams(location.search);query.delete('c');query.set('lang',getLanguage());for(const part of parts)query.set(part.id,state.selections[part.id]);history.replaceState(null,'',`${location.pathname}?${query}`);localStorage.setItem('blueblack-pen-combination-query',query.toString());dispatchEvent(new CustomEvent('combinationchange',{detail:{...state.selections}}));}
...
function renderPalette(){if(!elements.palette)return;const part=partById(state.activePartId);const list=colors.filter(color=>color.group===part.colorGroup);if(part.colorGroup==='metal'){elements.palette.replaceChildren(paletteGroup(t('metalColors'),list,part));return;}elements.palette.replaceChildren(paletteGroup(t('originalColors'),list.filter(color=>!color.isNew),part),paletteGroup(t('newColors'),list.filter(color=>color.isNew),part));}

function fillSummary(target){if(!target)return;target.replaceChildren(...parts.map(part=>{const color=colorById(state.selections[part.id]);const item=document.createElement('div');item.className='summary-item';item.dataset.partId=part.id;item.innerHTML=`<span class="swatch-dot" style="background:${color.hex}"></span><div><small>${localizePart(part).name}</small><b>${localizeColor(color)}</b></div>`;item.tabIndex=0;item.setAttribute('role','button');item.addEventListener('click',()=>selectPart(part.id,true));item.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();selectPart(part.id,true);}});return item;}));}
