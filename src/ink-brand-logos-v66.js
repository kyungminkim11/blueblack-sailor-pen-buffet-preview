const BRAND_META=[
  {keys:['3oysters','3 oysters','3오이스터스','쓰리오이스터스'],search:'3 Oysters',label:'3·OYSTERS',ko:'3오이스터스',country:'KR',tone:'navy',site:'https://3oysters.com/'},
...
function hostFor(site=''){
  try{return new URL(site).hostname.replace(/^www\./,'');}catch{return'';}
}
function domainLogo(site=''){
  const host=hostFor(site);return host?`https://logo.clearbit.com/${host}?size=256`:'';
}
function unavatarLogo(site=''){
  const host=hostFor(site);return host?`https://unavatar.io/${host}?fallback=false`:'';
}
function siteIcon(site=''){
  return site?`https://www.google.com/s2/favicons?sz=256&domain_url=${encodeURIComponent(site)}`:'';
}
function logoSources(meta){
  return[...new Set([meta.image,domainLogo(meta.site),unavatarLogo(meta.site),siteIcon(meta.site)].filter(Boolean))];
}
function logoMarkup(name,size='normal'){
  const meta=metaFor(name);const sources=logoSources(meta);const primary=sources.shift()||'';
  ...
  const queue=encodeURIComponent(JSON.stringify(sources));
  const image=primary?`<img src="${primary}" data-logo-queue="${queue}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">`:'';
...
function bindImageFallback(root=document){
  root.querySelectorAll('.ink-brand-logo img:not([data-bound])').forEach(image=>{
    ...
    image.addEventListener('load',()=>{const logo=image.closest('.ink-brand-logo');logo?.classList.add('has-image');logo?.classList.remove('image-failed');});
    image.addEventListener('error',()=>{
      let queue=[];try{queue=JSON.parse(decodeURIComponent(image.dataset.logoQueue||'%5B%5D'));}catch{}
      const next=queue.shift();image.dataset.logoQueue=encodeURIComponent(JSON.stringify(queue));
      if(next){image.src=next;return;}
      const logo=image.closest('.ink-brand-logo');logo?.classList.remove('has-image');logo?.classList.add('image-failed');
    });
    if(image.complete&&image.naturalWidth>0){const logo=image.closest('.ink-brand-logo');logo?.classList.add('has-image');logo?.classList.remove('image-failed');}
  });
}
function renderBrandDirectory(){
  const root=document.querySelector('.ink-store-quick');if(!root)return;
  const complete=root.querySelectorAll('[data-brand-card-v66]').length===BRAND_META.length;
  if(!complete)root.innerHTML=BRAND_META.map(meta=>`<button type="button" data-brand-card-v66 data-brand="${meta.search}" title="${meta.label}" aria-pressed="false">${logoMarkup(meta.search,'quick')}<span class="ink-brand-card-name">${meta.label}</span><small>${meta.country}</small></button>`).join('');
  if(!root.dataset.directoryBound){
    ...
    root.addEventListener('click',event=>{
      ...
      input.value=button.dataset.brand||'';input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();setTimeout(syncCurrent,20);
    });
  }
}
function enhanceResults(root=document){
  root.querySelectorAll('.ink-store-result .ink-store-brand:not([data-logo-ready])').forEach(label=>{
    const name=label.textContent.trim();const meta=metaFor(name);label.dataset.logoReady='true';
...
function enhanceCart(root=document){
  root.querySelectorAll('.ink-store-cart-main>b:not([data-logo-ready]),.ink-store-dialog-list b:not([data-logo-ready])').forEach(label=>{
    const full=label.textContent.trim();const brand=full.split('·')[0]?.trim()||full;label.dataset.logoReady='true';
...
function syncCurrent(){
  ...
  document.querySelectorAll('.ink-store-quick button[data-brand]').forEach(button=>{
    const meta=metaFor(button.dataset.brand||'');const active=Boolean(value)&&[meta.search,meta.ko,meta.label,...meta.keys].some(key=>normalize(key)===value);
