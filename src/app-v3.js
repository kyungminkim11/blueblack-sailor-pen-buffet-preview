...
function movePart(direction) {
  ...
  if (next >= 0 && next < parts.length) {
    selectPart(parts[next].id);
    $('.control-card').scrollIntoView({ behavior: 'smooth', block: 'start' });
  } else if (direction > 0) {
...
renderer.setPixelRatio(Math.min(devicePixelRatio, mobileMedia.matches ? 1.25 : 1.75));
...
function paintMesh(mesh, color, part, active) {
  ...
  material.emissiveIntensity = active ? 0.018 : 0;
...
function shareCombination() {
  ...
  if (navigator.share) {
    navigator.share(data).then(() => showToast(t('shared'))).catch(() => {});
  } else {
    navigator.clipboard.writeText(location.href).then(() => showToast(t('copied'))).catch(() => showToast(location.href));
  }
}
