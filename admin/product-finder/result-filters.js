...
function activeText(){const parts=[];const sortLabels={relevance:'추천순',price_asc:'낮은 가격순',price_desc:'높은 가격순',stock_desc:'재고 많은 순',name_asc:'상품명순'};const stockLabels={all:'전체 재고',in_stock:'재고 있음',out_of_stock:'재고 0개',unknown:'재고 정보 없음'};const materialLabels={gold:'금닙',steel:'스틸닙',other:'기타 닙'};parts.push(sortLabels[state.sort],stockLabels[state.stock]);if(state.brand)parts.push(ui.brand.selectedOptions[0]?.textContent||state.brand);if(state.type)parts.push(state.type);if(state.material)parts.push(materialLabels[state.material]);if(state.minPrice||state.maxPrice)parts.push('가격 범위');if(state.nibs.size)parts.push([...state.nibs].join('·'));return parts.join(' · ')}
function updateActiveText(){ui.activeText.textContent=activeText();const count=(state.sort!=='relevance'?1:0)+(state.stock!=='all'?1:0)+(state.brand?1:0)+(state.type?1:0)+(state.material?1:0)+(state.minPrice||state.maxPrice?1:0)+(state.nibs.size?1:0);ui.status.textContent=count?`${count}개 조건이 적용됩니다.`:'필터 없이 검색어 관련 상품을 추천순으로 표시합니다.'}

let timer=0;
function scheduleSearch(){updateActiveText();clearTimeout(timer);timer=setTimeout(()=>{if(document.querySelector('[data-panel="text"]')?.classList.contains('active')&&searchInput.value.trim().length>=2)searchButton.click()},180)}
...
ui.type.onchange=()=>{state.type=ui.type.value;if(state.type!=='만년필'&&state.material){state.material='';ui.material.value=''}scheduleSearch()};
ui.material.onchange=()=>{state.material=ui.material.value;if(state.material){state.type='만년필';ui.type.value='만년필'}scheduleSearch()};
...
ui.reset.onclick=()=>{state.sort='relevance';state.stock='all';state.brand='';state.type='';state.material='';state.minPrice='';state.maxPrice='';state.nibs.clear();ui.sort.value='relevance';ui.stock.value='all';ui.brand.value='';ui.type.value='';ui.material.value='';ui.minPrice.value='';ui.maxPrice.value='';ui.nibs.querySelectorAll('button').forEach(button=>button.classList.remove('active'));scheduleSearch()};
...
function badgeFor(product){if(!product?.nib_material)return null;const badge=document.createElement('em');badge.className=`nib-material-badge ${product.nib_material}`;badge.textContent=product.nib_material_label||({gold:'금닙',steel:'스틸닙',other:'기타·확인 필요'}[product.nib_material]||'닙 확인 필요');return badge}
function decorateResults(){
  ...
  if(!brand||brand.querySelector('.nib-material-badge'))return;
...
window.fetch=async(...args)=>{
  ...
  const isTextSearch=document.querySelector('[data-panel="text"]')?.classList.contains('active')&&query&&query===searchInput.value.trim();
  const targetUrl=isTextSearch?requestUrl.replace('internal_product_search_public','internal_product_search_filtered_v2_public'):requestUrl.replace('internal_product_search_public','internal_product_search_with_nib_public');
  const payload=isTextSearch?{p_query:query,p_limit:100,p_sort:state.sort,p_min_price:numberOrNull(state.minPrice),p_max_price:numberOrNull(state.maxPrice),p_nibs:[...state.nibs],p_stock:state.stock,p_brand:state.brand||null,p_product_type:state.type||null,p_nib_material:state.material||null}:{p_query:query,p_limit:body.p_limit||50};
