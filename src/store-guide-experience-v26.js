import { tourLanguage } from './store-tour-i18n.js?v=26';
...
const copy = COPY[tourLanguage()] || COPY.ko;
let enhanced = false;
let lastSelected = '';
let syncFrame = 0;
...
function localizeQuickNav() {
  ...
  if (!nav) return;
  Object.entries(copy.nav).forEach(([key, value]) => setText($(`[data-store-nav="${key}"]`, nav), value));
...
function showToast(message) {
  ...
  if (!toast) {
    ...
    $('.store-tour-view-card')?.append(toast);
  }
  setText(toast, message);
  clearTimeout(showToast.timer);
  showToast.timer = setTimeout(() => setText(toast, ''), 1800);
}
...
function selectedIndex(buttons) {
  return buttons.findIndex((button) => button.classList.contains('is-selected'));
}
...
function sync(root) {
  ...
  setText($('[data-tour-progress]', root), `${index + 1} / ${buttons.length}`);
  ...
  buttons.forEach((button, itemIndex) => {
    button.setAttribute('aria-current', itemIndex === index ? 'location' : 'false');
...
function requestSync(root) {
  if (syncFrame) return;
  syncFrame = requestAnimationFrame(() => sync(root));
}
...
function setPlanCollapsed(planCard, collapsed) {
  ...
  if (button) {
    ...
    setText(button, collapsed ? copy.plan : copy.hidePlan);
  }
}
...
function enhance(root) {
  ...
  const planCard = $('.store-tour-plan-card', root);
  const viewShell = $('.store-tour-view-shell', root);
  const list = $('[data-tour-list]', root);
  if (!viewCard || !planCard || !viewShell || !list) return false;
  enhanced = true;
  root.classList.add('is-store-tour-enhanced');

  const bar = document.createElement('div');
  bar.className = 'tour-experience-bar';
  bar.innerHTML = `<button type="button" data-tour-prev>← ${copy.prev}</button><div class="tour-experience-progress"><small>${copy.point}</small><strong data-tour-progress>1 / 11</strong></div><button type="button" data-tour-next>${copy.next} →</button>`;
  $('.store-tour-card-head', viewCard)?.after(bar);

  const fullscreen = document.createElement('button');
  fullscreen.type = 'button';
  fullscreen.className = 'tour-view-fullscreen';
  fullscreen.dataset.tourFullscreen = '';
  ...
  utility.innerHTML = `<button type="button" data-tour-share>${copy.share}</button><button type="button" data-tour-plan-jump>${copy.plan}</button>`;
  $('.store-tour-hint', viewCard)?.after(utility);

  const planHead = $('.store-tour-card-head', planCard);
  if (planHead) {
    const toggle = document.createElement('button');
    toggle.type = 'button';
    toggle.className = 'tour-plan-toggle';
    toggle.dataset.tourPlanToggle = '';
    toggle.setAttribute('aria-expanded', 'true');
    toggle.textContent = copy.hidePlan;
    planHead.append(toggle);
  }

  $('[data-tour-prev]', root)?.addEventListener('click', () => {
    const buttons = $$('[data-spot-id]', list);
    buttons[selectedIndex(buttons) - 1]?.click();
  });
  $('[data-tour-next]', root)?.addEventListener('click', () => {
    const buttons = $$('[data-spot-id]', list);
    buttons[selectedIndex(buttons) + 1]?.click();
  });
  fullscreen.addEventListener('click', () => document.fullscreenElement ? document.exitFullscreen?.() : viewShell.requestFullscreen?.());
  $('[data-tour-share]', root)?.addEventListener('click', shareCurrent);
  $('[data-tour-plan-jump]', root)?.addEventListener('click', () => {
    setPlanCollapsed(planCard, false);
  ...
  // Only selection class changes are observed. Watching text/child mutations caused
  // the previous progress update to recursively trigger itself and freeze mobile browsers.
  new MutationObserver(() => requestSync(root)).observe(root, {
    subtree: true,
    attributes: true,
    attributeFilter: ['class'],
  });
  document.addEventListener('fullscreenchange', () => setText(fullscreen, document.fullscreenElement ? copy.exitFullscreen : copy.fullscreen));
  requestSync(root);

  const params = new URLSearchParams(location.search);
  if (params.has('tour') && !location.hash && !root.dataset.autoFocused) {
    root.dataset.autoFocused = 'true';
    setTimeout(() => root.scrollIntoView({ behavior: 'smooth', block: 'start' }), 450);
  }
  return true;
}
