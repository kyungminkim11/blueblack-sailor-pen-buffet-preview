...
function addReadyBadge(){
  const stage=document.querySelector('#viewer-stage');
  if(!stage)return;
  let badge=stage.querySelector('.viewer-quality-badge');
  if(!badge){badge=document.createElement('div');badge.className='viewer-quality-badge';stage.append(badge);}
  badge.textContent=text().ready;
}
function addRetry(){
  const error=document.querySelector('#viewer-error');
  if(!error)return;
  let button=error.parentElement.querySelector('.viewer-error-action');
  if(!button){button=document.createElement('button');button.type='button';button.className='viewer-error-action';button.addEventListener('click',()=>location.reload());error.insertAdjacentElement('afterend',button);}
