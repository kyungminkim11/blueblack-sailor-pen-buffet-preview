...
const ui={sort:details.querySelector('#filterSort'),stock:details.querySelector('#filterStock'),brand:details.querySelector('#filterBrand'),type:details.querySelector('#filterType'),minPrice:details.querySelector('#filterMinPrice'),maxPrice:details.querySelector('#filterMaxPrice'),nibs:details.querySelector('#filterNibs'),status:details.querySelector('#filterStatus'),activeText:details.querySelector('#activeFilterText'),reset:details.querySelector('#filterReset')};

nibOptions.forEach(([value,label])=>{const button=document.createElement('button');button.type='button';button.className='nib-filter-button';button.dataset.value=value;button.textContent=label;button.onclick=()=>{state.nibs.has(value)?state.nibs.delete(value):state.nibs.add(value);button.classList.toggle('active',state.nibs.has(value));updateActiveText();scheduleSearch()};ui.nibs.append(button)});

function numberOrNull(value){const parsed=Number(value);return value!==''&&Number.isFinite(parsed)?parsed:null}
function activeText(){const parts=[];const sortLabels={relevance:'추천순',price_asc:'낮은 가격순',price_desc:'높은 가격순',stock_desc:'재고 많은 순',name_asc:'상품명순'};const stockLabels={all:'전체 재고',in_stock:'재고 있음',out_of_stock:'재고 0개',unknown:'재고 정보 없음'};parts.push(sortLabels[state.sort],stockLabels[state.stock]);if(state.brand)parts.push(ui.brand.selectedOptions[0]?.textContent||state.brand);if(state.type)parts.push(state.type);if(state.minPrice||state.maxPrice)parts.push('가격 범위');if(state.nibs.size)parts.push([...state.nibs].join('·'));return parts.join(' · ')}
function updateActiveText(){ui.activeText.textContent=activeText();const count=(state.sort!=='relevance'?1:0)+(state.stock!=='all'?1:0)+(state.brand?1:0)+(state.type?1:0)+(state.minPrice||state.maxPrice?1:0)+(state.nibs.size?1:0);ui.status.textContent=count?`${count}개 조건이 적용됩니다.`:'필터 없이 검색어 관련 상품을 추천순으로 표시합니다.'}
...
ui.type.onchange=()=>{state.type=ui.type.value;scheduleSearch()};
...
ui.reset.onclick=()=>{state.sort='relevance';state.stock='all';state.brand='';state.type='';state.minPrice='';state.maxPrice='';state.nibs.clear();ui.sort.value='relevance';ui.stock.value='all';ui.brand.value='';ui.type.value='';ui.minPrice.value='';ui.maxPrice.value='';ui.nibs.querySelectorAll('button').forEach(button=>button.classList.remove('active'));scheduleSearch()};
...
window.fetch=async(...args)=>{
  ...
  if(!isTextSearch)return originalFetch(...args);
  const filteredUrl=requestUrl.replace('internal_product_search_public','internal_product_search_filtered_public');
  const payload={p_query:query,p_limit:100,p_sort:state.sort,p_min_price:numberOrNull(state.minPrice),p_max_price:numberOrNull(state.maxPrice),p_nibs:[...state.nibs],p_stock:state.stock,p_brand:state.brand||null,p_product_type:state.type||null};
  return originalFetch(filteredUrl,{...options,body:JSON.stringify(payload)});
};
