const COPY = {
  ...
  en: {
    openMap: 'In-store map', title: 'In-store brand map', intro: 'Use the entrance and counter as landmarks to find each brand display.', largeView: 'Open large map', searchLabel: 'Find a brand', searchPlaceholder: 'e.g. Sailor, Lamy, Pilot', fit: 'Fit', fullscreen: 'Full screen', searchHint: 'Enter a brand name to highlight its display area.', found: (count) => `${count} display area${count === 1 ? '' : 's'} highlighted.`, missing: 'No matching display area was found.', legendBrand: 'Brand displays', legendTable: 'Tables & trial area', legendService: 'Counter & entrance', note: 'Display locations may change according to store operation and stock.'
  },
  ja: {
    openMap: '店内マップ', title: '店内ブランドマップ', intro: '入口とカウンターを目印に、各ブランドの売場をご確認ください。', largeView: '大きな地図を開く', searchLabel: 'ブランド検索', searchPlaceholder: '例：Sailor、Lamy、Pilot', fit: '全体表示', fullscreen: '全画面', searchHint: 'ブランド名を入力すると該当売場を強調表示します。', found: (count) => `${count}か所の売場を強調表示しました。`, missing: '一致する売場が見つかりませんでした。', legendBrand: 'ブランド売場', legendTable: 'テーブル・試筆', legendService: 'カウンター・入口', note: '売場位置は店舗運営や入荷状況により変更される場合があります。'
  },
  'zh-Hans': {
    openMap: '店内地图', title: '店内品牌地图', intro: '以入口和柜台为参照，快速查找各品牌陈列位置。', largeView: '打开大图', searchLabel: '查找品牌', searchPlaceholder: '例如 Sailor、Lamy、Pilot', fit: '适应', fullscreen: '全屏', searchHint: '输入品牌名称即可高亮对应陈列区域。', found: (count) => `已高亮 ${count} 个陈列区域。`, missing: '未找到匹配的陈列区域。', legendBrand: '品牌陈列', legendTable: '桌台与试写', legendService: '柜台与入口', note: '陈列位置可能会根据门店运营和到货情况调整。'
  },
  'zh-Hant': {
    openMap: '店內地圖', title: '店內品牌地圖', intro: '以入口和櫃檯為參照，快速查找各品牌陳列位置。', largeView: '開啟大圖', searchLabel: '查找品牌', searchPlaceholder: '例如 Sailor、Lamy、Pilot', fit: '適應', fullscreen: '全螢幕', searchHint: '輸入品牌名稱即可醒目顯示對應陳列區域。', found: (count) => `已醒目顯示 ${count} 個陳列區域。`, missing: '未找到相符的陳列區域。', legendBrand: '品牌陳列', legendTable: '桌台與試寫', legendService: '櫃台與入口', note: '陳列位置可能會依門市營運和到貨狀況調整。'
  }
};
...
function zones() { return mapDocument ? [...mapDocument.querySelectorAll('[data-brands]')] : []; }

function setResult(type, count = 0) {
  const copy = COPY[currentLanguage()] || COPY.ko;
  result.classList.remove('is-found', 'is-missing');
  if (type === 'found') { result.textContent = copy.found(count); result.classList.add('is-found'); }
  else if (type === 'missing') { result.textContent = copy.missing; result.classList.add('is-missing'); }
  else result.textContent = copy.searchHint;
}
...
function searchMap() {
  ...
  if (!query) { setResult('hint'); return; }
...
function applyPaperAisleGap(doc) {
  ...
  const entranceGroup = [...doc.querySelectorAll('[data-brands]')].find((group) => group.dataset.brands?.includes('Entrance'));
  ...
  innerGroups.forEach((group) => {
    ...
    text?.querySelectorAll('tspan').forEach((span) => span.setAttribute('x', '1090'));
  });

  [outerGroup, entranceGroup].forEach((group) => {
    const rect = group?.querySelector('rect');
    const text = group?.querySelector('text');
    if (!rect || !text) return;
    const centerY = Number(rect.getAttribute('y')) + Number(rect.getAttribute('height')) / 2;
    rect.setAttribute('x', '1140');
    rect.setAttribute('width', '30');
    text.setAttribute('x', '1155');
...
function applyZoom(next) { zoom = Math.min(2.2, Math.max(0.65, next)); canvas.style.minWidth = '0'; canvas.style.width = `${Math.round(baseWidth() * zoom)}px`; }
function fitMap() { zoom = 1; canvas.style.minWidth = '0'; canvas.style.width = '100%'; if (viewport) { viewport.scrollLeft = 0; viewport.scrollTop = 0; } }
async function toggleFullscreen() { if (!viewport) return; if (document.fullscreenElement) await document.exitFullscreen?.(); else await viewport.requestFullscreen?.(); }
